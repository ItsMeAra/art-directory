...
export default function ListingCard({ listing, activeTag, onTagSelect }: ListingCardProps) {
  ...
  return (
    <div className={`
      bg-white dark:bg-zinc-900 rounded-xl border flex flex-col gap-3 p-5
    ...
    `}>
      ...
      <div className='pt-2'>
        <h2 className="font-semibold text-gray-900 dark:text-zinc-100 text-lg leading-snug">{listing.name}</h2>
        {listing.location && (
          <span className="text-xs text-gray-400 dark:text-zinc-500 shrink-0 pt-0.5 py-1 block">{listing.location}</span>
        )}
        <p className="text-sm text-gray-500 dark:text-zinc-400 mt-1 line-clamp-3">{listing.description}</p>
      </div>
      ...
      <a
        ...
        className="mt-auto inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg bg-violet-500 text-white text-sm font-medium hover:bg-violet-600 dark:hover:bg-violet-400 transition-colors"
      >
        Visit
